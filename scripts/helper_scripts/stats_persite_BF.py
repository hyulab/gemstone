# Title: stats_persite_BF.py
# Description: This script is to calculate statistics (of variant density, variant quality, mean depth, allele frequency, ts/tv ratio, insertion/deletion lengths, variant type) on unfiltered VCF for visualization.
# Author: Siwei Chen
...
gendir = '../query/D3_output/{0}/unfiltered/general'.format(c['qid'])
...
quals= [float(i.split('\t')[2]) for i in open('{0}/tmp.lqual'.format(gendir)).readlines()[1:] if 'nan' not in i.split('\t')[2]]
...
for c in chrom:
    quals= [float(i.split('\t')[2]) for i in open('{0}/tmp.lqual'.format(gendir)).readlines()[1:] if i.split('\t')[0] == c and 'nan' not in i.split('\t')[2]]
...
def afSpectrum(chromosome):    # chrom need to be 1,2 instead of chr1,chr2
    af= []
    #noval= 0
...
open('{0}/mean_depth_hist.csv'.format(gendir),'w').write('POSITION,HEIGHT\n' + listToHistogram([float(i[:-1].split('\t')[2]) for i in open('{0}/tmp.ldepth.mean'.format(gendir)).readlines()[1:] if 'nan' not in i[:-1].split('\t')[2]],100))

# Generate data for each chromosome
for c in chrom:
    depths= [float(i.split('\t')[2]) for i in open('{0}/tmp.ldepth.mean'.format(gendir)).readlines()[1:] if i.split('\t')[0] == c and 'nan' not in i[:-1].split('\t')[2]]
