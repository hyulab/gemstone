# Title: af_sc_ANN.py
# Description: This script is to annotate and/or filter variants for allele frequency, variant consequence and location on genome build GRCh37/hg19.
# Author: Siwei Chen
...
fh_ANN.write('CHROM\tPOS\tID\tREF\tALT\tFILTER\tINDIVIDUAL_ID\tCONSEQUENCE\tPUTATIVE_IMPACT\tGENE_NAME\tENTREZ_ID\tENSEMBL_ID\tFEATURE_TYPE\tFEATURE_ID\tTRANSCRIPT_BIOTYPE\tHGVS.c\tHGVS.p\t' +
             ...
             'TAGC_AF\tTAGC_AC\tExAC_MEAN_COVERAGE\n') 
for var in fh_FM:
    # Parsing
    if var.split('\t')[7].split('|')[7] in c['transcript_biotype']:
        ...
        cur.execute('select AF_Adj,AF_AFR,AF_AMR,AF_EAS,AF_FIN,AF_NFE,AF_SAS, AF_OTH from ExAC_hg19 where (CHROM,POS,REF,ALT)= (%s, %s, %s, %s);', query)
        rows = cur.fetchall()
        if rows == ():
        ...
        cur.execute('select * from 1000G_hg19 where (CHROM,POS,REF,ALT)= (%s, %s, %s, %s);', query)
        rows = cur.fetchall()
        if rows == ():
        ...
        fh_ANN.write('\t'.join(chr_pos_id_ref_alt + fil + indv_id + cons_imp_gname_entr_ensg_ftype_fid_trans_hgvscp +
                               exac_All_AFR_AMR_EAS_FIN_NFE_SAS_OTH + esp_All_EA_AA + kg_All_AMR_AFR_EAS_EUR_SAS + tagc + exac_coverage) + '\n')

fh_ANN.close()

ann1_col = {'ExAC': '$40,$21,$22,$23,$24,$25,$26,$27,$28,', 'ESP6500': '$29,$30,$31,', '1000Genome': '$32,$33,$34,$35,$36,$37,', 'TAGC': '$38,$39,'}
