# Title: ddG.py
# Description: This script is to calculate ddG score for variants.
# Author: Siwei Chen
...
for var in fh_2ddg[1:]:
    consequence = var[:-1].split('\t')[7]
    uniprot = var[:-1].split('\t')[17]
    if consequence != 'missense_variant' or uniprot == '.':
        fh_ddg.write(var[:-1] + '\t.\n')
        continue
    else:
        aa_pos = int(var.split('\t')[16][5:-3])
        ...
        if 'min_cst_0.5.' + modelID + '_0001.pdb' not in fh_preminimized:
            ...
            aa_change = aa_code[var.split('\t')[16][2:5]] + '\t' + str(aa_pos - aa_begin + 1) + '\t' + aa_code[var.split('\t')[16][-3:]] + '\n'
